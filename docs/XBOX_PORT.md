...
### Phase 9: console-style interface (v1.1)
- **Button icons.** An Xbox atlas replaces the PS2 one behind the same `Ps2ButtonIcon` slots (Cross = A, Circle = B, L1/R1 = White/Black...), plus single D-pad directions for the Controls screen. A copy is compiled into the XBE because the console's `data\` folder is often older than the XBE.
...
The Xbox button icons, the crafting tab icons and the crafting groups table are generated by the scripts in `scripts/xbox/` from the Minecraft Legacy Console Edition files (graphics and recipe definitions). The generated headers embed those images in the build.
