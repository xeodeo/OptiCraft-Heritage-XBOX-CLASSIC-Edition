...
// Wide path -> narrow XAPI path. Xbox file names are ASCII; anything else
// becomes '_'. The game spells paths with '/', XAPI object paths need '\'.
...
// XboxXtl.h renames the XDK's 2003 Interlocked* intrinsic declarations so
// they do not clash with the VS2022 STL's; XDK inline helpers that expand to
// the renamed functions resolve here.
LONG WINAPI XdkInterlockedIncrement(LONG* p)
{
    return _InterlockedIncrement((volatile long*)p);
}
LONG WINAPI XdkInterlockedDecrement(LONG* p)
{
    return _InterlockedDecrement((volatile long*)p);
}
LONG WINAPI XdkInterlockedExchange(LONG* p, LONG v)
{
    return _InterlockedExchange((volatile long*)p, v);
}
LONG WINAPI XdkInterlockedExchangeAdd(LONG* p, LONG v)
{
    return _InterlockedExchangeAdd((volatile long*)p, v);
}
LONG WINAPI XdkInterlockedCompareExchange(LONG* p, LONG exchange, LONG comparand)
{
    return _InterlockedCompareExchange((volatile long*)p, exchange, comparand);
}
...
BOOL WINAPI CreateDirectoryA(const char*, void*);
BOOL WINAPI DeleteFileA(const char*);

// ---------------------------------------------------------------------------
// Shared-mode SRW locks: the spin lock above is exclusive-only; readers take
// it exclusively too (correct, just no reader concurrency).
// ---------------------------------------------------------------------------
void WINAPI AcquireSRWLockShared(volatile LONG* lock)
{
    AcquireSRWLockExclusive(lock);
}
BOOL WINAPI TryAcquireSRWLockShared(volatile LONG* lock)
{
    return TryAcquireSRWLockExclusive(lock);
}
void WINAPI ReleaseSRWLockShared(volatile LONG* lock)
{
    ReleaseSRWLockExclusive(lock);
}

// ---------------------------------------------------------------------------
// Two-phase one-time init (std::call_once). Same 0/1/2 state word as
// InitOnceExecuteOnce above: not run / running / done.
// ---------------------------------------------------------------------------
#define INIT_ONCE_CHECK_ONLY 0x1
#define INIT_ONCE_INIT_FAILED 0x4

BOOL WINAPI InitOnceBeginInitialize(volatile LONG* once, DWORD flags, BOOL* pending, void** ctx)
{
    if (ctx) {
        *ctx = nullptr;
    }
    ...
    for (;;) {
        const LONG prev = _InterlockedCompareExchange(once, 1, 0);
        if (prev == 0) {
            *pending = TRUE;  // caller runs the initializer, then completes
            return TRUE;
        }
        if (prev == 2) {
            *pending = FALSE;
...
BOOL WINAPI InitOnceComplete(volatile LONG* once, DWORD flags, void*)
{
    _InterlockedExchange(once, (flags & INIT_ONCE_INIT_FAILED) ? 0 : 2);
    return TRUE;
}

// The VS2022 STL routes std::call_once through these msvcp exports.
int WINAPI __std_init_once_begin_initialize(void** once, DWORD flags, int* pending, void** ctx)
{
    BOOL p = FALSE;
    const BOOL ok = InitOnceBeginInitialize((volatile LONG*)once, flags, &p, ctx);
    *pending = p;
    return ok;
}
int WINAPI __std_init_once_complete(void** once, DWORD flags, void* ctx)
{
    return InitOnceComplete((volatile LONG*)once, flags, ctx);
}

// ---------------------------------------------------------------------------
// Wide file-system entry points used by the UCRT's _mkdir / remove.
// ---------------------------------------------------------------------------
BOOL WINAPI CreateDirectoryW(const WCHAR* path, void* sa)
{
    char narrow[MAX_PATH];
    if (!narrowPath(path, narrow, sizeof(narrow))) {
        SetLastError(ERROR_INVALID_PARAMETER);
        return FALSE;
    }
    return CreateDirectoryA(narrow, sa);
}
BOOL WINAPI DeleteFileW(const WCHAR* path)
{
    char narrow[MAX_PATH];
    if (!narrowPath(path, narrow, sizeof(narrow))) {
        SetLastError(ERROR_INVALID_PARAMETER);
...
// One CPU: every affinity request "succeeds" with the only mask there is.
uintptr_t WINAPI SetThreadAffinityMask(HANDLE, uintptr_t)
{
    return 1;
}
...
BOOL WINAPI RemoveDirectoryA(const char*);

// UCRT _rmdir / _wrmdir.
BOOL WINAPI RemoveDirectoryW(const WCHAR* path)
{
    char narrow[MAX_PATH];
    if (!narrowPath(path, narrow, sizeof(narrow))) {
        SetLastError(ERROR_INVALID_PARAMETER);
